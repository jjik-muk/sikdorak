...
public class DistanceCalcUtil {
    ...
    private static double calculateDistanceInMeter(double desX, double desY, double curX, double curY) {
        double distance = Math.sin(Math.toRadians(desY)) * Math.sin(Math.toRadians(curY)) + Math.cos(Math.toRadians(desY)) * Math.cos(Math.toRadians(curY)) * Math.cos(Math.toRadians(desX - curX));
